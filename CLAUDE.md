...
- **Context extraction (`context.py`, used by Code mode):** `extract_context(request, workspace, home)` builds ranked, bounded, provenance-tagged `ContextFinding`s from two sources, never executing anything it reads. An always-on instruction floor: agent-instruction files (workspace `CLAUDE.md`/`AGENTS.md`/`CODEX.md` + global `~/.claude/CLAUDE.md`, category `agent_instruction`) plus `.mcp.json` (server names ONLY — never values; skipped when no `mcpServers` map). Each instruction file is chunked by `_chunk_instruction` — an always-kept overview (title/preamble, `MAX_INSTRUCTION_OVERVIEW_CHARS`) plus the markdown sections most relevant to the request's query terms, capped at `MAX_INSTRUCTION_CHARS_PER_FILE` (3 KB) so a large `CLAUDE.md` adds a few KB, not tens (a latency fix); code-fenced `#` lines never split a section. Layered on top, query-driven grep snippets: `extract_query_terms(request)` (`enhancement/query.py`) splits the dictated request on camelCase/snake_case and drops stopwords, then `retrieve_snippets(terms, workspace)` (`enhancement/retrieval.py`) greps the repo for those terms with identifier-aware boundaries (`_` separates, so `transcription` matches `transcription_language`) — a Python `os.walk`+`re` engine is the always-available path, with an optional `rg` fast-path when present — returning bounded snippets attributed by `path:line` (a markdown section for doc hits, the matching line ± context for code hits). Each finding carries `source_path`/`category`/`confidence`. Agent-instruction findings are always kept (chunked per file, bounded by `MAX_FINDINGS`); the ranked grep-snippet tail is ordered by request/text token overlap and trimmed to `MAX_TOTAL_CHARS`. Security: a secret-file denylist (`.env`, `*.key`, `*.pem`, …) is never read regardless of term matches; every file access is symlink-guarded, containment-checked, byte-capped, and NUL-stripped; logging is content-free (paths/counts only, never matched text); it never raises — on any failure it returns whatever was gathered. `ContextFinding` shape, trust tiers, the Engineering Task contract, and deterministic `sources_of_truth` are all unchanged by this.
